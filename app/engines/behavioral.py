...
# Weights sum to exactly 1.00 — matches the target decomposition.
_W_ACCEPTANCE = 0.22
_W_PRESSURE = 0.18
_W_STRUCTURE = 0.19
...
_STABILITY_REFERENCE_BARS = 20.0        # bars held before stability maxes out
_TRANSITION_RISK_BASE = {"Expansion": 15.0, "Negotiation": 45.0, "Compression": 55.0}
_HISTORICAL_SIMILARITY_REFERENCE = 100.0  # occurrences before similarity maxes out
...
def _transition_risk_value(data: BehavioralData) -> Tuple[float, str]:
    ...
    # Displayed value is inverted so higher = lower risk, consistent with
    # every other factor in this score being higher-is-better.
...
def score_behavioral(
    ...
    # Confidence blends BO/MC lifecycle maturity (as before) with how many
    # of the optional, database-backed inputs were actually supplied —
    # missing inputs pull confidence down rather than being silently
    # papered over by their neutral fallback values.
    ...
    return ScoreExplanation(
        ...
        methodology=(
            ...
            "Stability (9%), Transition Risk (8%, displayed inverted so "
            "higher = lower risk), and Historical Similarity (10%). Each "
            "factor has its own Research Card (app/research.py) explaining "
            "its definition, research basis, calculation, evidence, "
